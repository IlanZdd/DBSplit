					...
					for (int j : overlappingArray) {
						...
						switch (executeWhat) {
							case 0 -> {
								...
								DBSplit.DBSplit(DBMS, sv, user, password, DB, DB1, DB2, percent, splitType, percentOverlapping, graph, true);
								System.out.println("\tAlgorithm completed in " + roundDecimal(((double) System.currentTimeMillis() - execTime) / 1000, 1000) + "s.");
								execTime = System.currentTimeMillis();
								com.DBSplit.DBSBaseline.DBSplit.DBSplit(DBMS, sv, user, password, DB, DB1, DB2, percent, splitType, percentOverlapping, graph, true);
