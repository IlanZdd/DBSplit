package com.DBSplit.DBSKnapsack;

import Graph.ForeignKeyColumn;
import com.DBSplit.Other.DBConnection;
import com.DBSplit.Other.MainDebug;

import java.sql.ResultSet;
import java.sql.Statement;
import java.util.*;

import static com.DBSplit.DBSKnapsack.DBSplit.graph;
...
public class Knapsack {
    ...
    private static void gatherer(String table, ForeignKeyColumn fk, int ucn) {
        ...
        try {
            ...
            if (DBSplit.DBMS.equalsIgnoreCase("mysql"))
                query+= DBSplit.DB1 + ".";
            query+= table;
    ...
    protected static KS_Return entryPointMidSQLite(String table, List<ForeignKeyColumn> allFKs, int n, int m, int u, int c) {
        ...
        try {
            ...
            DBConnection.setConn(DBSplit.DBMS, DBSplit.sv, DBSplit.username, DBSplit.password, DBSplit.DB1);
            ...
            DBConnection.closeConn();
            DBConnection.setConn(DBSplit.DBMS, DBSplit.sv, DBSplit.username, DBSplit.password, DBSplit.DB2);
            st = DBConnection.getConn().createStatement();
            rs = st.executeQuery(query);
            ...
            for (int i = 0; i < mapCommons.size(); ++i) {
                if (chosenBits.get(i)) {
                    commonFKs += DBSplit.listToString(mapCommons.get(fksCommon.get(i)), 0, fksCommonCount.get(i)) + ", ";
            ...
            for (int i = 0; i < mapDB1.size(); ++i) {
                if (chosenBits.get(i)) {
                    deleteNFromDB1 += DBSplit.listToString(mapDB1.get(fksCommon.get(i)), 0, fksCommonCount.get(i)) + ", ";
                    if (mapDB2.containsKey(fksCommon.get(i))) {
            ...
            for (int i = 0; i < mapDB2.size(); ++i) {
               if (chosenBits.get(i))
                    deleteMFromDB2 += DBSplit.listToString(mapDB2.get(fksCommon.get(i)), 0, fksCommonCount.get(i)) + ", ";
            }
