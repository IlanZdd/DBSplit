package com.DBSplit.DBSFromHigher;
...
import static com.DBSplit.DBSFromHigher.DBSplit.graph;
    ...
    public static KS_Return SQLiteMid(String table, List<ForeignKeyColumn> allFKs, int n, int m, int u, int c) {
        ...
        try {
            ...
            for (int i = 0; i < sortedFks.size() && upTo < u+c; ++i) {
                ...
                commonFKs += DBSplit.printStr(sortedFks.get(i).getRowIds(), 0, (u+c) - upTo) + ", ";
            ...
            for (int i = 0; i < sortedFks.size() && upTo < n; ++i) {
                String key = sortedFks.get(i).getForeignKey();
                deleteNfromDB1 += DBSplit.printStr(sortedFks.get(i).getRowIds(), 0, n - upTo) + ", ";
                upTo += sortedFks.get(i).size();
            ...
            for (int i = 0; i < sortedFks.size() && upTo < m; ++i) {
                deleteMfromDB2 += DBSplit.printStr(sortedFks.get(i).getRowIds(), 0, m - upTo) + ", ";
                upTo += sortedFks.get(i).size();
            }
